  ...
  function build(intelligence) {
    ...
    return {
      primaryCard: {
        ...
        severityBadge: primary.severity ? String(primary.severity).toUpperCase() : 'NEUTRAL',
      ...
      quickActionCards: quickParams.map((item) => ({
        ...
        applyAction: item.apply_action || null,
        meta: item.parameter ? `Quick Apply can set ${item.parameter} = ${item.suggested_value}` : 'Available for direct apply',
      })),
      manualGuidanceCards: manualGuidance.map((item) => ({
        ...
        tone: 'slate',
        applyAction: item.apply_action || null,
        meta: 'AI can apply this suggestion, show the diff, and let you retest explicitly.',
      })),
      heroActions: [
