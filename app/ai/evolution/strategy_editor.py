...
def _build_mutation_prompt_v2(
    ...
    return (
        ...
        f"- Primary weakness: {rcd.get('primary_failure_label', 'unknown')}\n"
        f"- Causal chain: {' -> '.join(step.get('finding', '') for step in rcd.get('causal_chain', [])[:3])}\n"
...
async def mutate_strategy(
    ...
    role_overrides = (
        {"code_gen": model, "reasoner": model, "explainer": model} if model else None
    )
    models = await fetch_free_models(provider)
    prompt = _build_mutation_prompt_v2(
        strategy_name=strategy_name,
        source_code=source_code,
        analysis=analysis,
        fitness=fitness,
